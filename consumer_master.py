...
try:
	...
	cursor.execute("select distinct a.mobile from \
					...
					WHERE (date_updated>DATE_SUB("+str(from_time)+",INTERVAL "+str(time_interval)+") \
					OR date_created>DATE_SUB("+str(from_time)+",INTERVAL "+str(time_interval)+")) \
					AND product_id=24  \
					UNION( \
					SELECT trim(username) AS mobile FROM shmart_consumer.users \
					WHERE created>DATE_SUB("+str(from_time)+",INTERVAL "+str(time_interval)+") \
					OR modified>DATE_SUB("+str(from_time)+",INTERVAL "+str(time_interval)+")) \
					UNION( \
					...
					WHERE a.creationTime>DATE_SUB("+str(from_time)+",INTERVAL  "+str(time_interval)+") \
					OR a.updateTime > DATE_SUB("+str(from_time)+",INTERVAL  "+str(time_interval)+")) \
					UNION( \
					...
					inner JOIN shmart_consumer.users b ON a.consumer_id=b.id \
					WHERE a.creation_time>DATE_SUB("+str(from_time)+",INTERVAL  "+str(time_interval)+") \
					OR a.updation_time > DATE_SUB("+str(from_time)+",INTERVAL  "+str(time_interval)+")) \
					UNION( \
					...
					WHERE a.date_updated>DATE_SUB("+str(from_time)+",INTERVAL  "+str(time_interval)+") \
					AND a.product_id=24 \
					AND b.product_id=24) \
					\
					UNION( \
					...
					inner JOIN shmart_consumer.users b ON a.user_id=b.id \
					WHERE a.creationTime>DATE_SUB("+str(from_time)+",INTERVAL  "+str(time_interval)+") \
					OR a.updateTime>DATE_SUB("+str(from_time)+",INTERVAL  "+str(time_interval)+"))) a")
